...
namespace ChessGame {
    class Functions {
    public:
        static sf::Vector2f convertToPosition(int r, int c, float squareSize);
        static std::pair<int, int> convertToSquare(sf::Vector2f position, float squareSize);
        static int convertToNumber(sf::Vector2f position, float squareSize);
